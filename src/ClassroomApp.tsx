...
import { calculateBirthProfile, birthProfileFacts, type BirthProfileResult } from './lib/scoring/birthProfile';
...
function ClassroomApp() {
  if (window.location.pathname.startsWith('/report/')) return <ClassroomReportPage />;
  return <ClassroomJourney />;
}
...
function ClassroomJourney() {
  ...
  const [lifePathResonance, setLifePathResonance] = useState<LifePathResonance | undefined>();
  const [riasecAnswers, setRiasecAnswers] = useState<Partial<Record<`q${string}`, RiasecAnswer>>>({});
  const [subjectiveDriver, setSubjectiveDriver] = useState<RiasecCode | undefined>();
  const [talentUsage, setTalentUsage] = useState<TalentUsage | undefined>();
  ...
  const riasecResult = useMemo(() => answeredCount === RIASEC_QUESTIONS.length ? scoreRiasec(RIASEC_QUESTIONS, riasecAnswers) : null, [answeredCount, riasecAnswers]);
  ...
  function ensureLogin() {
    ...
    return auth.status === 'authenticated' || auth.status === 'mock';
  }
  ...
  function answerRiasec(answer: RiasecAnswer) {
    ...
    if (!question) return;
    const next = { ...riasecAnswers, [question.id]: answer };
  ...
  function goPreviousQuestion() {
    ...
    const keysToKeep = RIASEC_QUESTIONS.slice(0, answeredCount - 1).map((q) => q.id);
    const next: Partial<Record<`q${string}`, RiasecAnswer>> = {};
    for (const key of keysToKeep) if (riasecAnswers[key]) next[key] = riasecAnswers[key];
    setRiasecAnswers(next);
  }
  ...
  function togglePriority(priority: Priority) {
    ...
    if (priorities.length < 2) setPriorities([...priorities, priority]);
  }

  async function finalize() {
    if (!lifePath || !birthProfile || !riasecResult || !lifePathResonance || !subjectiveDriver || !talentUsage || priorities.length === 0) return;
    ...
    try {
      ...
      const birthSignature = calculateBirthSignature(birthDate);
      const { assessment } = await createAssessment({
        birthDate,
        subjectId: subjectResponse.subject.subjectId,
        assessmentMode,
        lifePath,
        birthProfile,
        birthSignature,
  ...
  if (step === 'landing') return (
    <main className="classroom-shell"><section className="classroom-panel classroom-hero">
      <p className="classroom-eyebrow">天賦原動力 · 課堂探索</p>
      <h1>做一點，看一點，聊一點</h1>
      <p className="classroom-lede">今天不急著把你分析完。我們分三段，先看生命靈數、再看活動偏好，最後把線索放在一起。</p>
      <div className="classroom-stage-row"><span>① 出生結構</span><span>② 活動偏好</span><span>③ 當下狀況</span></div>
      <button className="classroom-primary" type="button" onClick={() => { if (auth.status === 'unauthenticated') window.location.assign('/api/auth/line/start'); else setStep('birthday'); }} disabled={auth.status === 'loading'}>{auth.status === 'loading' ? '正在準備…' : '開始我的探索'}</button>
      <a className="classroom-link" href="/report/latest">查看我上次的完整報告</a>
    </section></main>
  );

  if (step === 'birthday') return (
    <main className="classroom-shell"><section className="classroom-panel classroom-panel--narrow">
      <p className="classroom-eyebrow">第一階段 · 出生結構</p>
      <h1>先算出你的生命靈數</h1>
      <p className="classroom-lede">這只是第一面鏡子。先看看有沒有共鳴，等等再由老師帶大家一起解讀。</p>
      ...
      <div className="classroom-choice-grid classroom-choice-grid--two">
        <button className={assessmentMode === 'self' ? 'is-selected' : ''} onClick={() => setAssessmentMode('self')} type="button">我自己</button>
        <button className={assessmentMode === 'co_present' ? 'is-selected' : ''} onClick={() => setAssessmentMode('co_present')} type="button">陪另一位一起探索</button>
      </div>
      {dateError ? <p className="classroom-error">{dateError}</p> : null}
      <button className="classroom-primary" type="button" onClick={revealLifePath} disabled={!birthDate}>看看我的數字</button>
    </section></main>
  );

  if (step === 'life' && lifePath && lifeContent && birthFacts) return (
    <main className="classroom-shell"><section className="classroom-panel classroom-panel--wide">
      <p className="classroom-eyebrow">第一階段完成</p>
      ...
      <div className="classroom-two-cards">
        <article><small>比較容易有精神</small><p>{lifeContent.strengths[0]}</p></article>
        <article><small>比較容易被耗掉</small><p>{lifeContent.drains[0]}</p></article>
      </div>
      <div className="classroom-birth-preview">
        ...
        <article><small>目前階段</small><b>{birthFacts.current_stage.number ?? '—'}｜{birthFacts.current_stage.theme ?? birthFacts.current_stage.label}</b><span>{birthFacts.current_stage.keywords?.join('・') ?? '先把它當成現在值得探索的主題'}</span></article>
      </div>
      <div className="classroom-resonance"><h2>剛才的描述，有幾分像你？</h2><div className="classroom-choice-grid classroom-choice-grid--three">
        {([['high', '很像'], ['partial', '有一些像'], ['low', '目前不太像']] as Array<[LifePathResonance, string]>).map(([value, label]) => <button key={value} className={lifePathResonance === value ? 'is-selected' : ''} onClick={() => setLifePathResonance(value)} type="button">{label}</button>)}
      </div></div>
  ...
  if (step === 'riasec-intro') return (
    <main className="classroom-shell"><section className="classroom-panel classroom-panel--wide">
      <p className="classroom-eyebrow">第二階段 · RIASEC 活動偏好</p>
      <h1>不是看你「是什麼人」，而是看你喜歡怎麼做事情</h1>
      <div className="classroom-riasec-six">
        {(Object.keys(RIASEC_META) as RiasecCode[]).map((code) => <article key={code}><b>{code}</b><strong>{RIASEC_META[code].name}｜{RIASEC_META[code].verb}</strong><span>{RIASEC_ACTIVITY_DESCRIPTIONS[code]}</span></article>)}
      </div>
      <p className="classroom-lede">共 18 題，請憑第一直覺作答。沒有標準答案，選完會自動進到下一題。</p>
  ...
  if (step === 'riasec') {
    ...
    return <main className="classroom-shell"><section className="classroom-panel classroom-panel--narrow classroom-question">
      ...
      <p className="classroom-eyebrow">第 {answeredCount + 1} / {RIASEC_QUESTIONS.length} 題</p>
      <h1>{question.text}</h1>
  ...
  if (step === 'riasec-result' && riasecResult) return (
    <main className="classroom-shell"><section className="classroom-panel classroom-panel--wide">
      <p className="classroom-eyebrow">第二階段完成</p>
      <h1>你的活動偏好 Top 3</h1>
      <p className="classroom-lede">字母只是代號，真正重要的是後面的完整名稱與生活場景。</p>
      ...
      <div className="classroom-combo"><small>你的組合</small><strong>{riasecResult.top3Code}｜{riasecResult.top3.map((code) => `${RIASEC_META[code].name.replace('型', '')}・${RIASEC_META[code].verb}`).join(' × ')}</strong><p>不要只看第一名。你真正容易投入的方式，常常藏在三個偏好的組合裡。</p></div>
  ...
  if (step === 'reality') return (
    <main className="classroom-shell"><section className="classroom-panel classroom-panel--wide">
      <p className="classroom-eyebrow">第三階段 · 現在的你</p>
      <h1>最後用 2 分鐘，把測驗拉回真實生活</h1>
      ...
      <button className="classroom-primary" disabled={!subjectiveDriver || !talentUsage || priorities.length === 0 || isSaving} type="button" onClick={() => void finalize()}>{isSaving ? '正在保存你的探索快照…' : '整理我的天賦快照'}</button>
    </section></main>
  );

  if (step === 'snapshot' && completedAssessment && riasecResult && lifePath && lifeContent) {
    const top3IncludesDriver = subjectiveDriver ? riasecResult.top3.includes(subjectiveDriver) : false;
    const insights = [
      top3IncludesDriver && subjectiveDriver ? `不同角度重複出現「${RIASEC_META[subjectiveDriver].name}｜${energyLabel(subjectiveDriver)}」的線索，值得回想你過去在哪些真實情境最有成就感。` : subjectiveDriver ? `你的 RIASEC Top 3 與主觀能量照到不同方向。這不是矛盾，而是值得追問：什麼事情吸引你，和什麼事情真正讓你有精神，可能不完全相同。` : '',
      talentUsage && talentUsage <= 40 ? `你只給目前的天賦使用感 ${talentUsage}%。也許問題不只是能力，而是現在的環境沒有讓這些偏好充分被使用。` : `你給目前的天賦使用感 ${talentUsage}%。可以繼續觀察：哪些工作或生活情境，最容易讓這個比例往上。`,
    ...
    return <main className="classroom-shell"><section className="classroom-panel classroom-panel--wide">
      ...
      <div className="classroom-snapshot-grid">
        <article><small>出生結構</small><strong>{lifePath.value}｜{lifeContent.label}</strong><p>{lifeContent.coreMotivation}</p></article>
        <article><small>活動偏好</small><strong>{riasecResult.top3Code}</strong><p>{riasecResult.top3.map((code) => `${RIASEC_META[code].name}｜${RIASEC_META[code].verb}`).join('・')}</p></article>
        <article><small>本人能量</small><strong>{energyLabel(subjectiveDriver)}</strong><p>天賦使用感 {talentUsage}%</p></article>
        <article><small>現在最關注</small><strong>{priorities[0]}</strong><p>{priorities[1] ?? '先從一個重點開始'}</p></article>
      </div>
...
function ClassroomReportPage() {
  ...
  async function load() {
    ...
    try {
      ...
      if (!latest.assessment) { setStatus('missing'); return; }
  ...
  useEffect(() => {
    if (auth.status === 'authenticated' || auth.status === 'mock') void load();
    else if (auth.status === 'unauthenticated') setStatus('missing');
  }, [auth.status]);
  ...
  if (auth.status === 'loading' || status === 'loading') return <main className="classroom-shell"><section className="classroom-panel classroom-panel--narrow"><h1>正在讀取你的完整報告…</h1></section></main>;
  if (auth.status === 'unauthenticated') return <main className="classroom-shell"><section className="classroom-panel classroom-panel--narrow"><h1>先用同一個 LINE 帳號登入</h1><p className="classroom-lede">登入後，就能找回你在課堂中保存的探索結果。</p><a className="classroom-primary classroom-anchor" href="/api/auth/line/start">使用 LINE 登入</a></section></main>;
  if (status === 'missing' || !assessment) return <main className="classroom-shell"><section className="classroom-panel classroom-panel--narrow"><h1>還找不到完整結果</h1><p className="classroom-lede">請先完成一次「天賦原動力」課堂探索。</p><a className="classroom-primary classroom-anchor" href="/">開始探索</a></section></main>;
  if (status === 'error') return <main className="classroom-shell"><section className="classroom-panel classroom-panel--narrow"><h1>目前無法讀取報告</h1><button className="classroom-primary" onClick={() => void load()} type="button">重新讀取</button></section></main>;
  ...
  return <main className="classroom-shell"><section className="classroom-panel classroom-panel--wide classroom-report">
    <p className="classroom-eyebrow">我的天賦探索報告</p>
    <h1>{assessment.lifePath.value}｜{life.label}</h1>
    ...
    <details open={Boolean(report)}><summary>04｜AI 綜合整理</summary><div className="classroom-report-body">{report ? <>
      ...
      <h3>出生結構摘要</h3><p>{report.birth_profile_summary}</p>
      <h3>活動偏好與原動力</h3><p>{report.motivator_summary}</p>
      <h3>可能的內在張力</h3><ul>{report.possible_tensions.map((item) => <li key={item}>{item}</li>)}</ul>
      <h3>可能還沒充分使用的部分</h3><p>{report.unused_potential}</p>
      <h3>可以先探索的方向</h3><ul>{report.exploration_directions.map((item) => <li key={item}>{item}</li>)}</ul>
